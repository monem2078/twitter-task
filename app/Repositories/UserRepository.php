...
class UserRepository extends BaseRepository
{
    ...
    {
        $user = auth()->user()->followers()->attach(['following_id' => $user->id]);
    ...
    {
        $model = $this->model->select('tweets.id', 'followings.image', 'followings.name')
        ...
        if (app()->getLocale() == 'ar') {
            $model->selectRaw('tweets.body_ar');
        } else {
            $model->selectRaw('tweets.body_en');
        }
